...
public class MD5Util {
  ...
  /**
   * JDK1.4中不支持以MappedByteBuffer类型为参数update方法，并且网上有讨论要慎用MappedByteBuffer， 原因是当使用 FileChannel.map
   * 方法时，MappedByteBuffer 已经在系统内占用了一个句柄， 而使用 FileChannel.close 方法是无法释放这个句柄的，且FileChannel有没有提供类似
   * unmap 的方法， 因此会出现无法删除文件的情况。
   *
   * <p>不推荐使用
   *
   * @param file
